$(document).ready(function(){
	...
	var genSource = function(){
		...
		$("#form_config, #source").val($temptxt.html().replace(/\n\ \ \ \ \ \ \ \ \ \ \ \ /g,"\n"));
	}
	...
	$("#target").delegate(".component", "click", function(e){
		...
		$.each(valtypes, function(i,e){
			...
			if(required==="#required"){
				$(".popover #required").attr("checked", "checked");
			}
		});
		...
		$(".popover").delegate(".btn-info", "click", function(e){
			...
			$.each(inputs, function(i,e){
				...
				if(vartype==="placeholder"){
					...
					var required = $active_component.find('input, textarea');
					var label = $active_component.find('[data-valtype="label"]');
					console.log(label);
					if($(e).is(":checked")){
